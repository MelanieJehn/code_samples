...
class CorrectnessEvaluator(BaseEvaluator):
    ...
    async def aevaluate(
        ...
        **kwargs: Any,
        ...
        # This needs to be done as the llm sometimes does not generate the answer in the correct format
        print(eval_response)
        answer_components = eval_response.lstrip().split('\n')
        if len(answer_components) < 2:
            if len(answer_components[0]) > 4:
                # we separate at first whitespace as there is no newline between score and reasoning
                score_str, reasoning = answer_components[0].split(' ', maxsplit=1)
                try:
                    score = float(score_str)
                except ValueError:
                    score = 4.0
                    reasoning = score_str + ' ' + reasoning
            else:
                # we only have a score, add unknown reasoning
                score = float(answer_components[0])
                reasoning = "unknown"
        # make sure there is a \n between number and feedback!
        else:
            score, reasoning = self.parser_function(eval_response.lstrip())
